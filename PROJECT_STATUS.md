...
### 2026-04-23 新增「喜欢做的事」字段

#### 背景
现有匹配维度（性别、日期、吸烟、作息、噪音）只解决"不出事"。加入这个字段是向"化学反应层"迈出第一步——让两个陌生人在第一次联系时有一个自然的话题切入点，也为未来的兴趣向量积累数据。

#### 改动内容

**前端（index.html）**
- 表单新增「讲一件你喜欢做的事」选填 textarea，100字限制
- hint 文案：「匹配成功后会展示给对方」
- 结果页新增「喜欢做的事」展示行，对方有填则显示，没填则隐藏

**后端（api.py）**
- 新用户记录保存 `hobby` 字段；已有用户再次提交时同步更新
- 所有 match_info 返回结构中加入 `hobby` 字段
